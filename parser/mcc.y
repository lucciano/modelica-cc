...
%define MEMBERS \
          ...
          void yyerror(char const *s) { cerr << "error encountered at line: "<<lexer.lineno()<<" last word parsed: "<<lexer.YYText()<<"\n";} \
...
%type <expression> expression simple_expression primary opt_comp_call subscript function_argument opt_expression opt_condition_attribute opt_in named_argument opt_assing opt_equal_exp
...
opt_external_function_call:
    /* empty */
  | component_reference TOKEQUAL TOKID TOKOPAREN expression_list TOKCPAREN  {}
...
import_clause:
  TOKIMPORT opt_import comment { $$ = newAST_ImportClause(NULL); }
;
...
when_statement:
  TOKWHEN expression TOKTHEN statement_list opt_else_when_list TOKEND TOKWHEN { $$ = newAST_Statement_When($2,$4,$5); }
;
...
function_arguments: 
  ...
  | named_arguments                          { $$ = newAST_ExpressionList(); /* TODO */ }
;
